...
from bot.app.services.Exam.timer import (
    ExamTimerPreparations,
)  # Замените на фактическое имя вашего модуля
...
async def test_resulted_timer_mode1_with_students():
    ...
    with patch(
        "bot.app.repositories.CRUD.student_exams.get_all_students",
        ...
        assert (
            result_text
            == "Время на одного студента: 35, Общее время экзамена: 120, Время на подготовку: 15, Количество студентов: 3"
        )
...
async def test_resulted_timer_mode2_with_students():
    with patch(
        ...
        assert (
            result_text
            == "Время на одного студента: 30, Общее время экзамена: 75, Время на подготовку: 15, Количество студентов: 2"
        )
