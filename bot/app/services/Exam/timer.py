...
class ExamTimerPreparations:
    ...
    async def resulted_timer(
        self,
    ) -> Tuple[str, Tuple[Optional[int], Optional[int], Optional[int], int, List[str]]]:
        ...
        if self.exam_time:  # Если выбран режим 1
            if len(self.students) > 0:  # Проверяем, есть ли студенты
                if self.exam_time > self.preparation_time:
                    self.time_per_student = (
                        self.exam_time - self.preparation_time
                    ) / len(
                        self.students
                    )  # Используем дробное деление

                    # Условие для минимального времени на студента
                    if self.time_per_student < 3:
                        self.time_per_student = (
                            3  # Устанавливаем минимальное время на студента
                        )

                else:
                    self.time_per_student = (
                        0  # Или установите другое значение по умолчанию
                    )

                result_text = (
                    f"Время на одного студента: {self.time_per_student:.2f}, "
            ...
            if len(self.students) > 0:  # Проверяем, есть ли студенты
                self.exam_time = (
                    self.time_per_student * len(self.students)
                ) + self.preparation_time
                result_text = (
                    f"Время на одного студента: {self.time_per_student}, "
                    f"Общее время экзамена: {self.exam_time}, "
                    f"Время на подготовку: {self.preparation_time}, "
                    f"Количество студентов: {len(self.students)}"
                )
        ...
        return "Не указаны корректные параметры для расчета времени.", (
            None,
            None,
            None,
            0,
