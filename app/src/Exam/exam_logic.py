...
class Timer():
    ...
    def __init__(self, theor_time: int = 0, practice_time: int = 0, students_quantity: int = 0):
    ...
    def set_timer(self, students: list, bot: Bot, message):
        ...
        while timers_time > 0:
            if self.__EXAM_TIME:
                ...
                bot.send_message(message.from_chad.id, f"Текущий участник {students[students_id_counter]}")
    ...
    def thirteen_mins_passed(self, time: int):
        return True if time + self.__THIRTEEN_MINUTES == self.__base_time_in_sec else False

    def time_has_passed(self, time: int):
        return True if time + self.__TIME_HAS_PASSED == self.__base_time_in_sec else False
